  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const doctorsData = await DoctorService.getDoctorsHomePage()
      ...
      <Stack direction="row" spacing={3}>
        <TextField
          ...
          InputProps={{
            ...
            endAdornment: <SearchIcon style={{ color: '#2FC1A6', fontSize: '30px', fontWeight: '800' }} />
          }}
        ...
        <FormControl style={textStyle}>
          ...
          <Select labelId="specialty-select-label" value={setSpecialties} label="Chuyên khoa">
        ...
        <FormControl style={textStyle}>
          ...
          <Select labelId="specialty-select-label" value={setSpecialties} label="Địa điểm">
            {specialties.map((specialty) => (
              <MenuItem key={specialty.id} value={specialty.name}>
                {specialty.name}
              </MenuItem>
